...
path_data = path + 'data_SouthAmerica/'
...
for f in f_array_plot:
    ...
    np.save(path_data + f'SouthAmerica_K_f{f}.npy', Ktemp)
    ...
    np.save(path_data + f'SouthAmerica_lambdaM_f{f}.npy', lambdaM)
    print("\t time to compute lambdaM: ", measure_time.time() - t0)
    
    t0 = measure_time.time()
    rho = model.simulate(NDyn, Nsteps, dt, Ktemp, cvec, evec, rho0 = rho_0)
    np.save(path_data + f'SouthAmerica_rhostat_f{f}.npy', rho[-1])
    print("\t time to simulate: ", measure_time.time() - t0)
    
np.save(path_data + 'SouthAmerica_f_array.npy', f_array_plot)