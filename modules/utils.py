...
def find_distance_matrix(connected_network):
    """
    Finds the distance matrix between patches.
    ...
    igraph_network = igraph.Graph.from_networkx(connected_network)
    distmatrix = np.array(igraph.Graph.shortest_paths(igraph_network))
    unique_dist = np.unique(distmatrix)

    return distmatrix, unique_dist
